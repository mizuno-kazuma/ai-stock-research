...
推奨タブ（`/recommendations`）の母集団は当日の推奨カードではない。`scores_daily` の当日断面（スコア済みユニバース）である。推奨カードは銘柄に付く追加情報であり、一覧に出る資格条件にしない。

| 表示 | 条件 |
| --- | --- |
| `display_tier='core'` | 推奨カードがあり、`RANK_FILL` ではない |
| `display_tier='fill'` | 推奨カードがあり、`RANK_FILL` を含む（件数補充） |
| `display_tier='score_only'` | 推奨カードがない。定量スコアのみ |

カードがない行でも `name_local`（会社名）とスコア（`total_score`、なければ `quant_score`）は必須。ティッカーだけの行は出さない。会社名が証券マスタに無いときだけティッカーを名前の代わりに使う。

**既定表示の下限**: 絞り込みが未指定のとき、ダッシュボード「今週の注目」は `min(10, universe_size)` 件を出す。優先順は core（承認/修正）→ fill → 却下カード → `score_only`（`total_score` 降順）。Critic が全件却下しても空にしない。却下を承認に書き換えない。

利用者がレビュー結果・行動・確信度・「カードあり」などで母集団を狭めた結果が 10 件未満になるのは許容する。

全銘柄に LLM の強気/弱気論拠は付けない。カードがある銘柄だけフルカードを展開する。条件の細かい組み立てと保存はスクリーナー（§9）の役割のままとする。
